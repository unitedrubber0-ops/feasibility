...
@app.route('/export-pdf-with-balloons', methods=['POST'])
def export_pdf_with_balloons_handler():
    """
    Receives a source file and balloon data, draws the balloons on the PDF,
    and returns the modified PDF.
    """
    if 'sourceFile' not in request.files or 'balloonData' not in request.form:
        return jsonify({"error": "Missing source file or balloon data"}), 400

    source_file = request.files['sourceFile']
    balloon_data_str = request.form['balloonData']
    
    try:
        balloon_data = json.loads(balloon_data_str)
        file_stream = io.BytesIO(source_file.read())
        file_type = source_file.filename.split('.')[-1]
        ...
        for balloon in balloon_data:
            page_num = balloon.get("page")
            x = balloon.get("x")
            y = balloon.get("y")
            number = str(balloon.get("number"))
            
            if not all([page_num, x, y, number]):
                continue

            page = doc.load_page(page_num - 1)
            
            # Define balloon properties
            center = fitz.Point(x, y)
            radius = 16
            color = (1, 0, 0) # Red
            
            # Draw the circle
            circle = fitz.Circle(center=center, radius=radius)
            shape = page.new_shape()
            shape.draw_circle(circle)
            shape.finish(color=color, fill=(1, 1, 1), width=1.5, fill_opacity=0.5) # White fill, 50% transparent
            
            # Add the number inside the balloon
            text_pos = fitz.Point(x - (len(number) * 4), y + 5) # Simple centering
            shape.insert_text(text_pos, number, fontsize=12, fontname="helv-bold", color=(0,0,0)) # Black, bold text
        ...
        return send_file(
            pdf_buffer,
            as_attachment=True,
            download_name='ballooned_report.pdf',
            mimetype='application/pdf'
        )

    except Exception as e:
        print(f"Error drawing balloons on PDF: {e}")
        return jsonify({"error": f"Failed to generate ballooned PDF: {str(e)}"}), 500
